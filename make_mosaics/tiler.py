...
def image_boxes(img, res):
    ...
    for y in range(0, img.shape[0], shift[1]):
        for x in range(0, img.shape[1], shift[0]):
            boxes.append(
                {"img": img[y : y + res[0], x : x + res[1]], "pos": (x, y)}
            )
...
def most_similar_tile(box_mode_freq, tiles):
    if not box_mode_freq[0]:
        ...
        for t in tiles:
            dist = (
                1 + color_distance(box_mode_freq[0], t["mode"])
            ) / box_mode_freq[1]
...
def place_tile(img, box):
    ...
    if OVERLAP_TILES or not np.any(img_box[mask]):
        img_box[mask] = box["tile"][: img_box.shape[0], : img_box.shape[1], :][
            mask
        ]
...
def create_tiled_image(boxes, res, render=False):
    ...
    for box in tqdm(
        sorted(boxes, key=lambda x: x["min_dist"], reverse=OVERLAP_TILES)
    ):
        place_tile(img, box)
        if render:
