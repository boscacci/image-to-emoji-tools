...
class InstagramScraper(object):
    ...
    def authenticate_with_login(self):
        ...
        if login_text.get("authenticated") and login.status_code == 200:
            ...
            if "checkpoint_url" in login_text:
                ...
                for count, error in enumerate(
                    login_text["errors"].get("error")
                ):
                    count += 1
                    self.logger.debug(
    ...
    def set_last_scraped_timestamp(self, username, timestamp):
        if self.latest_stamps_parser:
            if not self.latest_stamps_parser.has_section(
    ...
    def worker_wrapper(self, fn, *args, **kwargs):
        try:
            ...
            self.logger.debug(
                "Exception in worker thread", exc_info=sys.exc_info()
            )
            raise
                ...
                for item in tqdm.tqdm(
                    ...
                    if self.filter_locations:
                        if (
                            ...
                            or item.get("location").get("id")
                            not in self.filter_locations
                        ):
                            continue
                    if (
                        (
                            item["is_video"] is False
                            and "image" in self.media_types
                        )
                        or (
                            item["is_video"] is True
                            and "video" in self.media_types
                        )
    ...
    def query_hashtag_gen(self, hashtag):
        return self.__query_gen(
            QUERY_HASHTAG, QUERY_HASHTAG_VARS, "hashtag", hashtag
        )
    ...
    def augment_node(self, node):
        ...
        if node["is_video"] and "video_url" in node:
            ...
            if details:
                if (
                    "__typename" in details
                    and details["__typename"] == "GraphVideo"
                ):
                    node["urls"] = [details["video_url"]]
                elif (
                    "__typename" in details
                    and details["__typename"] == "GraphSidecar"
                ):
                    urls = []
                    for carousel_item in details["edge_sidecar_to_children"][
                        "edges"
                    ]:
                    ...
                    node["urls"] = [
                        self.get_original_image(details["display_url"])
                    ]
    ...
    def get_profile_pic(self, dst, executor, future_to_item, user, username):
        ...
        if (
            ...
            or os.path.isfile(dst + "/" + item["urls"][0].split("/")[-1])
            is False
        ):
            for item in tqdm.tqdm(
                [item],
    ...
    def get_profile_info(self, dst, username):
        ...
        try:
            profile_info = {
                ...
                "posts_count": user_info["edge_owner_to_timeline_media"][
                    "count"
                ],
    ...
    def get_stories(self, dst, executor, future_to_item, user, username):
        ...
        if self.logged_in and (
            "story-image" in self.media_types
            or "story-video" in self.media_types
        ):
        ...
        if resp is not None:
            ...
            if (
                ...
                higlight_stories_ids = [
                    item["node"]["id"]
                    for item in retval["data"]["user"]["edge_highlight_reels"][
                        "edges"
                    ]
                ]
    ...
    def extract_tags(self, item):
        ...
        if "caption" in item and item["caption"]:
            ...
            caption_text = item["edge_media_to_caption"]["edges"][0]["node"][
                "text"
            ]
    ...
    def templatefilename(self, item):
        ...
        for url in item["urls"]:
            ...
            try:
                ...
                customfilename = str(
                    template.format(**template_values) + extension
                )
    ...
    def is_new_media(self, item):
        ...
        return (
            current_timestamp > 0
            and current_timestamp > self.last_scraped_filemtime
        )
...
def main():
    ...
    if args.followings_input:
        ...
        if args.followings_output:
            with open(
                scraper.destination + scraper.followings_output, "w"
            ) as file:
